...
Trying tailwind, because why not?
...
I'm gonna snapshot commit because there's a few things in flight here:

- I'm migrating out a big tangled mess of logic into two new classes related to tournaments and score calculation within the rounds of tournaments. This is going well.

- My primary reason for the above is I need better control to be able to differentiate over the whole no stats situation. As it existed before, all these calculations were sorta wound up on one another in a single-threaded iterative script. Now with the new class structure I'll use this granular control to calculate the numbers in different ways based on athlete-round characteristics.

Anyway, that's where I'm at. I'm outputting the results object which currently is empty on stats and makes. I've also done little-to-no cleanup of duplicated code, so Code.gs is gonna go down in size a lot.
