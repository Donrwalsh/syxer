...
class PlayerSheet {
  ...
  emptyScorecard(division, round) {
    ...
    [
      ...
      sheet
        .getRange(`${ROUND_ALPHA[round == 12 ? 4 : round - 1]}${int}`)
        .setValues([[0]])
    );
  ...
  getAthleteLineup() {
    return [
      {
        division: "MPO #1",
        athlete: this.roster.getRange("B3").getDisplayValue(),
      },
      {
        division: "MPO #2",
        athlete: this.roster.getRange("B4").getDisplayValue(),
      },
      {
        division: "MPO #3",
        athlete: this.roster.getRange("B5").getDisplayValue(),
      },
      {
        division: "FPO #1",
        athlete: this.roster.getRange("C3").getDisplayValue(),
      },
      {
        division: "FPO #2",
        athlete: this.roster.getRange("C4").getDisplayValue(),
      },
      {
        division: "FPO #3",
        athlete: this.roster.getRange("C5").getDisplayValue(),
      },
    ];
  }
  ...
  getCurrentPointsSum(tab, round) {
    ...
    let range = this.ss
      .getSheetByName(tab)
      .getRange(`${ROUND_ALPHA[annoying - 1]}4:${ROUND_ALPHA[annoying - 1]}23`)
      .getValues();
    return range.reduce((acc, cv) => acc + (parseInt(cv) || 0), 0);
  }

  getEventTotalByEventName(eventName) {
    //TODON untested
    return this.ss
      .getSheetByName("Event Totals")
      .getRange(
        TOURNAMENTS.find((tournament) => tournament.name == eventName).cell
      )
      .getDisplayValue();
  }
  ...
  getMatchups() {
    ...
    [
      ...
      matchups.push(this.matchup.getRange(`D${int}`).getDisplayValue())
    );
    return matchups;
  }
  ...
  writeStatsToScorecard(stats, division, round) {
    ...
    // Strokes
    sheet
      .getRange(`${roundAlpha}4`)
      .setValues([[stats.strokes.doubleBogey.toString()]]);
    sheet
      .getRange(`${roundAlpha}5`)
      .setValues([[stats.strokes.bogey.toString()]]);
    sheet
      .getRange(`${roundAlpha}6`)
      .setValues([[stats.strokes.par.toString()]]);
    sheet
      .getRange(`${roundAlpha}7`)
      .setValues([[stats.strokes.birdie.toString()]]);
    sheet
      .getRange(`${roundAlpha}8`)
      .setValues([[stats.strokes.eagle.toString()]]);
    sheet
      .getRange(`${roundAlpha}9`)
      .setValues([[stats.strokes.albatross.toString()]]);
    ...
    sheet
      .getRange(`${roundAlpha}14`)
      .setValues([[stats.stats.parked.toString()]]);
    ...
    sheet
      .getRange(`${roundAlpha}17`)
      .setValues([[stats.stats.noStats.toString()]]);

    // Makes
    sheet.getRange(`${roundAlpha}20`).setValues([[stats.makes.c1x.toString()]]);
    sheet
      .getRange(`${roundAlpha}21`)
      .setValues([[stats.makes.c1xBonus.toString()]]);
    sheet.getRange(`${roundAlpha}22`).setValues([[stats.makes.c2.toString()]]);
    sheet
      .getRange(`${roundAlpha}23`)
      .setValues([[stats.makes.c2Bonus.toString()]]);
    sheet
      .getRange(`${roundAlpha}24`)
      .setValues([[stats.makes.throwIns.toString()]]);
  }
