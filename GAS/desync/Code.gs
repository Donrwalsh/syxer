// v1.09c - cowboy coded bullshit + angular app work
let layoutData;
...
function doTheThing() {
  ...
  var oneToTwentyOne = [...Array(21)].map((_, i) => i + 1);

  for (const ps of playerSheets) {
    let selections = [];
    for (const tournId of oneToTwentyOne) {
      selections.push({
        id: tournId,
        mpo1: ps.sheet.getAthleteFromStats(tournId, 1),
        mpo2: ps.sheet.getAthleteFromStats(tournId, 2),
        mpo3: ps.sheet.getAthleteFromStats(tournId, 3),
        fpo1: ps.sheet.getAthleteFromStats(tournId, 4),
        fpo2: ps.sheet.getAthleteFromStats(tournId, 5),
        fpo3: ps.sheet.getAthleteFromStats(tournId, 6),
      });
    }

    result.push({
      player: ps.name,
      athleteSelections: selections,
    });
  ...
  for (let thing in result) {
    console.log(JSON.stringify(thing.athleteSelections));
  }

  // K15-K20, K28-K33, K41-K46
}
...
function roundStandings() {
  ...
  for (const playerName of playerSheets.map((sheet) => sheet.name)) {
    var sheet = playerSheets.find(
      (playerSheet) => playerSheet.name == playerName
    ).sheet;
    placement.push({
      name: playerName,
  ...
  for (let i = 0; i < placement.length; i++) {
    ...
    standings.writeToPlace(
      i + 1,
      placement[i].name,
      placement[i].record,
      placement[i].pointsFor,
      placement[i].pointsAgainst
    );
...
function calculatePointsAgainst(teamName, playerSheets) {
  ...
  const pastTournaments = TOURNAMENTS.filter(
    (tournament) => tournament.end < todaysDate
  );
  const matchups = playerSheets
    .find((playerSheet) => playerSheet.name == teamName)
    .sheet.getMatchups();
      ...
      ctrl.config.rounds.forEach((round) => {
        ...
        try {
          if (shouldGetData) {
            ...
            console.log(
              `${lineup.athlete} data for round ${round} already present. Skipping`
            );
          }
        } catch (e) {
          const errorMessage = `[${ps.getName()}]  Encountered error obtaining ${
            lineup.athlete
          }'s stats for round ${round}. Error: ${e} `;
...
function obtainAthleteStats(athleteName, tournamentId, round, division) {
  ...
  if (
    ...
    if (!noStats) {
      ...
      result.makes.c1xBonus =
        c1xPossible == result.makes.c1x ? result.makes.c1x : 0;
      result.makes.c2Bonus =
        c2Possible == result.makes.c2 ? result.makes.c2 : 0;
    }
...
function obtainAthleteData(athleteName, tournId, round, division) {
  if (!athleteData[division][round]) {
    ...
    var url = `https://www.pdga.com/apps/tournament/live-api/live_results_fetch_round?TournID=${tournId}&Division=${division}&Round=${
      round == 4 &&
      (tournId == 88286 ||
        tournId == 89546 ||
        tournId == 88293 ||
